  ...
  unsafe {
    if ReadFile(pipe, Some(&mut buffer), Some(&mut bytes_read), None).is_ok() {
      if bytes_read > 0 {
        ...
        // Parse data and convert to UIAF
        let data = &buffer[..bytes_read as usize];
        
        #[cfg(feature = "protobuf")]
        info!("Parsing as protobuf data");
        
        #[cfg(not(feature = "protobuf"))]
        info!("Protobuf not available, expecting JSON data");
        
        match crate::yae::proto::parse_yae_protobuf(data) {
          Ok(uiaf_data) => {
