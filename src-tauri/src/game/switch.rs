...
mod tests {
  use super::{
    PersistedSdk, PersistedSwitchPlan, collect_delete_files, extract_sdk_zip,
    load_or_create_switch_journal, load_persisted_switch_plan, persist_plan, record_switch_cache,
    sdk_cache_hit, sdk_decompress_budget, unique_sdk_packages, verify_staged_sdk,
  };
  use crate::game::{hoyoplay::ChannelSdkPackage, model::SchemeId};
  ...
  #[test]
  fn extracts_sdk_zip_with_bilibili_cef_entry_count() {
    let root = TempRoot::new();
    let zip_path = root.0.join("sdk.zip");
    ...
    let mut total = 0_u64;
    for index in 0..80 {
      let name = format!("YuanShen_Data/Plugins/BLPlatform64/locales/{index}.pak");
      zip.start_file(&name, options).unwrap();
    ...
    extract_sdk_zip(&zip_path, &staging, total).unwrap();
    assert!(staging.join("YuanShen_Data/Plugins/BLPlatform64/locales/79.pak").is_file());
  }

  #[test]
  fn decompress_budget_allows_hoyoplay_underreported_size() {
    let declared = 169_976_064_u64;
    let actual = 195_265_400_u64;
    assert!(actual > declared);
    assert!(sdk_decompress_budget(declared) >= actual);
    assert_eq!(sdk_decompress_budget(u64::MAX), super::MAX_SDK_DECOMPRESSED_BYTES);
  }

  #[test]
  fn extracts_when_zip_exceeds_declared_decompressed_size() {
    let root = TempRoot::new();
    let zip_path = root.0.join("sdk.zip");
    write_zip(&zip_path, &[("YuanShen_Data/Plugins/PCGameSDK.dll", b"0123456789")]);
    let staging = root.0.join("staging");
    fs::create_dir_all(&staging).unwrap();
    extract_sdk_zip(&zip_path, &staging, sdk_decompress_budget(8)).unwrap();
    assert_eq!(
      fs::read(staging.join("YuanShen_Data/Plugins/PCGameSDK.dll")).unwrap(),
      b"0123456789"
    );
  }

  #[test]
  fn rejects_sdk_zip_over_entry_limit() {
    let root = TempRoot::new();
    let zip_path = root.0.join("sdk.zip");
    let file = fs::File::create(&zip_path).unwrap();
    let mut zip = zip::ZipWriter::new(file);
    let options =
      zip::write::SimpleFileOptions::default().compression_method(zip::CompressionMethod::Deflated);
    for index in 0..=super::MAX_SDK_ZIP_ENTRIES {
      zip.start_file(format!("{index}.bin"), options).unwrap();
      zip.write_all(b"x").unwrap();
    }
    zip.finish().unwrap();
    let staging = root.0.join("staging");
    fs::create_dir_all(&staging).unwrap();
    assert_eq!(
      extract_sdk_zip(&zip_path, &staging, 1024).unwrap_err(),
      "渠道 SDK 压缩包条目数超过安全上限"
    );
  }

  #[test]
  fn rejects_zip_slip_sdk_entries() {
    let root = TempRoot::new();
    let zip_path = root.0.join("bad.zip");
    write_zip(&zip_path, &[("../escape.dll", b"bad")]);
    let staging = root.0.join("staging");
    fs::create_dir_all(&staging).unwrap();
    assert!(extract_sdk_zip(&zip_path, &staging, 1024).is_err());
    assert!(!root.0.join("escape.dll").exists());
  }

  #[test]
  fn verifies_sdk_pkg_version_before_commit() {
    let root = TempRoot::new();
    let staging = root.0.join("staging");
    fs::create_dir_all(staging.join("YuanShen_Data/Plugins")).unwrap();
    let dll = b"sdk-bytes";
    fs::write(staging.join("YuanShen_Data/Plugins/PCGameSDK.dll"), dll).unwrap();
    let md5 = {
      use md5::Digest;
      format!("{:x}", md5::Md5::digest(dll))
    };
    fs::write(
      staging.join("sdk_pkg_version"),
      format!(
        "{{\"remoteName\":\"YuanShen_Data/Plugins/PCGameSDK.dll\",\"md5\":\"{md5}\",\"fileSize\":{}}}\n",
        dll.len()
      ),
    )
    .unwrap();
    let package = PersistedSdk {
      version: "5.0.4".to_string(),
      pkg_version_file_name: "sdk_pkg_version".to_string(),
      md5: "0".repeat(32),
      size: 1,
    ...
    let staged = verify_staged_sdk(&staging, &package).unwrap();
    assert!(staged.contains_key("YuanShen_Data/Plugins/PCGameSDK.dll"));
    assert!(staged.contains_key("sdk_pkg_version"));
    fs::write(staging.join("YuanShen_Data/Plugins/PCGameSDK.dll"), b"tampered").unwrap();
    assert!(verify_staged_sdk(&staging, &package).is_err());
  }

  fn sample_sdk(md5: &str, size: u64) -> PersistedSdk {
    PersistedSdk {
      version: "5.0.4".to_string(),
      pkg_version_file_name: "sdk_pkg_version".to_string(),
      md5: md5.to_string(),
  ...
  fn switch_plan(
    ...
    PersistedSwitchPlan {
      ...
      source_channel: if source == SchemeId::CnOfficial { 1 } else { 14 },
      source_sub_channel: if source == SchemeId::CnOfficial { 1 } else { 0 },
      target_channel: if target == SchemeId::CnOfficial { 1 } else { 14 },
      target_sub_channel: if target == SchemeId::CnOfficial { 1 } else { 0 },
      sdk,
      source_sdk,
  ...
  #[test]
  fn unique_sdk_packages_include_source_when_target_sdk_is_absent() {
    let md5 = "a".repeat(32);
    let plan = switch_plan(
      Uuid::new_v4().to_string(),
      SchemeId::CnBilibili,
      SchemeId::CnOfficial,
      Some(sample_sdk(&md5, 4)),
      None,
    );
    let packages = unique_sdk_packages(&plan);
    assert_eq!(packages.len(), 1);
    assert_eq!(packages[0].md5, md5);
  }

  #[test]
  fn unique_sdk_packages_count_source_and_target() {
    let plan = switch_plan(
      Uuid::new_v4().to_string(),
      SchemeId::CnOfficial,
      SchemeId::CnBilibili,
      Some(sample_sdk(&"a".repeat(32), 4)),
      Some(sample_sdk(&"b".repeat(32), 8)),
    );
    assert_eq!(unique_sdk_packages(&plan).len(), 2);
  }

  #[test]
  fn persists_bilibili_to_official_journal_after_caching_source_sdk() {
    let root = TempRoot::new();
    let bytes = b"sdk!";
    let md5 = {
      use md5::Digest;
    ...
    let plan = switch_plan(
      ...
      SchemeId::CnBilibili,
      SchemeId::CnOfficial,
      Some(sample_sdk(&md5, bytes.len() as u64)),
      None,
    );
    let mut journal = load_or_create_switch_journal(&root.0, &plan).unwrap();
    assert_eq!(journal.planned_steps, 1);
    assert_eq!(journal.total_count, 1);
    assert_eq!(journal.committed_step, 0);
    record_switch_cache(&mut journal, &root.0, &unique_sdk_packages(&plan));
    journal.state = crate::game::model::PackageTaskState::ReadyToApply;
    crate::game::journal::persist(&root.0, &journal).expect("B 服回官服缓存源 SDK 后应能写入日志");
    assert_eq!(journal.committed_step, 1);
    assert_eq!(journal.owned_cache_files, vec![md5.clone()]);
    assert_eq!(journal.downloaded_bytes, bytes.len() as u64);
    assert_eq!(journal.planned_steps, 1);
  }

  #[test]
  fn loads_switch_plan_after_installation_plan_is_replaced() {
    let root = TempRoot::new();
    let first = switch_plan(
      Uuid::new_v4().to_string(),
      SchemeId::CnOfficial,
      SchemeId::CnBilibili,
      None,
    ...
    persist_plan(&root.0, &first).unwrap();
    load_or_create_switch_journal(&root.0, &first).unwrap();
    let second = switch_plan(
      Uuid::new_v4().to_string(),
      SchemeId::CnOfficial,
      SchemeId::CnBilibili,
      None,
    ...
    persist_plan(&root.0, &second).unwrap();
    let loaded = load_persisted_switch_plan(&root.0, first.plan_id()).unwrap();
    assert_eq!(loaded.plan_id(), first.plan_id());
    assert_eq!(loaded.sdk.as_ref().map(|package| package.size), Some(4));
  }

  #[test]
  fn journal_copy_survives_missing_installation_plan() {
    let root = TempRoot::new();
    let plan = switch_plan(
      Uuid::new_v4().to_string(),
      SchemeId::CnBilibili,
    ...
    persist_plan(&root.0, &plan).unwrap();
    load_or_create_switch_journal(&root.0, &plan).unwrap();
    fs::remove_dir_all(root.0.join("switch")).unwrap();
    let loaded = load_persisted_switch_plan(&root.0, plan.plan_id()).unwrap();
