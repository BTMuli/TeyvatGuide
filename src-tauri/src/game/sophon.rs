...
mod tests {
  ...
  use crate::game::{
    ...
    model::SchemeId,
    sophon::{get_decoded_build, get_decoded_patch_build},
  };

  #[test]
  fn rejects_chunk_outside_asset() {
  ...
  #[test]
  #[ignore = "只读访问官方 HoyoPlay/Sophon，用于协议变更冒烟验证"]
  fn decodes_current_official_game_manifest() {
    tauri::async_runtime::block_on(async {
      let client = create_http_client().unwrap();
      let branches = get_game_branches(&client, SchemeId::CnOfficial).await.unwrap();
      let build = get_decoded_build(&client, &branches.main, &[]).await.unwrap();
      assert_eq!(build.tag, branches.main.tag);
      assert_eq!(build.manifests.len(), 1);
      assert!(!build.manifests[0].data.assets.is_empty());
      if let Some(source_tag) = branches.main.diff_tags.first() {
        let patch =
          get_decoded_patch_build(&client, &branches.main, source_tag, &[]).await.unwrap();
        assert_eq!(patch.tag, branches.main.tag);
        assert_eq!(patch.manifests.len(), 1);
      }
    });
  }
}
