...
pub fn locate_executables() -> Vec<String> {
  #[cfg(target_os = "windows")]
  {
    locate_windows_unity_log_executables()
  }
  #[cfg(not(target_os = "windows"))]
  {
    Vec::new()
...
#[cfg(target_os = "windows")]
fn locate_windows_unity_log_executables() -> Vec<String> {
  let mut found = Vec::new();
  for log_path in unity_log_paths() {
    let Ok(bytes) = fs::read(&log_path) else {
      continue;
    };
    for executable in extract_yuanshen_executables(&String::from_utf8_lossy(&bytes)) {
      if found.iter().any(|existing: &String| existing.eq_ignore_ascii_case(&executable)) {
        continue;
      }
...
#[cfg(target_os = "windows")]
fn unity_log_paths() -> Vec<PathBuf> {
  let Some(home) = std::env::var_os("USERPROFILE") else {
    return Vec::new();
  };
  let local_low = PathBuf::from(home).join("AppData").join("LocalLow").join("miHoYo");
  vec![
    local_low.join("原神").join("output_log.txt"),
    local_low.join("Genshin Impact").join("output_log.txt"),
  ]
}
...
fn extract_yuanshen_executables(content: &str) -> Vec<String> {
  ...
  let needle = "yuanshen_data";
  let mut executables = Vec::new();
  let mut from = 0;
  while let Some(relative) = lower[from..].find(needle) {
    let index = from + relative;
    from = index + needle.len();
    let Some(executable) = executable_from_data_prefix(&content[..index]) else {
      continue;
    };
    if !Path::new(&executable).is_file() {
      continue;
    }
    if executables.iter().any(|existing: &String| existing.eq_ignore_ascii_case(&executable)) {
      continue;
    }
...
#[cfg(target_os = "windows")]
fn executable_from_data_prefix(prefix: &str) -> Option<String> {
  ...
  while index >= 2 {
    ...
    let mut path = prefix[index - 1..].replace('/', "\\");
