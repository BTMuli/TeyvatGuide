...
mod tests {
  use super::{
    DRAFT_SCHEMA_VERSION, InstallDraft, InstallDraftState, abandon_published_draft, build_config,
    has_published_installation, list_draft_summaries, marker_nonce, persist_draft,
  };
  ...
  #[test]
  fn config_is_deterministic_and_nonce_is_256_bit() {
    assert_eq!(
      build_config(SchemeId::CnOfficial, "1.2.3"),
      build_config(SchemeId::CnOfficial, "1.2.3")
    );
    assert_eq!(marker_nonce().len(), 64);
  }

  #[test]
  fn abandons_published_draft_without_removing_game_root() {
    let root = std::env::temp_dir().join(format!("teyvat-guide-installer-{}", Uuid::new_v4()));
    let task_root = root.join("tasks");
    let library_root = root.join("library");
    let game_root = library_root.join("Genshin");
    let draft_id = Uuid::new_v4().to_string();
    fs::create_dir_all(&game_root).unwrap();
    fs::create_dir_all(task_root.join("install-drafts")).unwrap();
    let marker_nonce = "a".repeat(64);
    let staging_root =
      library_root.join(format!(".teyvatguide-install-{draft_id}-{}", &marker_nonce[..12]));
    let draft = InstallDraft {
      schema_version: DRAFT_SCHEMA_VERSION,
      ...
      target_file_id: 0,
      expected_executable: game_root.join("YuanShen.exe").to_string_lossy().into_owned(),
      marker_nonce,
      scheme: SchemeId::CnOfficial,
      audio_languages: vec!["zh-cn".to_string()],
      state: InstallDraftState::Published,
      plan_id: Some("plan-id".to_string()),
      target_tag: Some("1.0.0".to_string()),
    ...
    persist_draft(&task_root, &draft).unwrap();

    let drafts = list_draft_summaries(&task_root).unwrap();
    assert_eq!(drafts.len(), 1);
    assert_eq!(drafts[0].draft_id, draft_id);
    assert_eq!(drafts[0].install_root, game_root.to_string_lossy());

    let mut pre_publish_draft = draft.clone();
    pre_publish_draft.state = InstallDraftState::Downloading;
    assert!(!has_published_installation(&pre_publish_draft).unwrap());

    let summary = abandon_published_draft(&task_root, &draft_id).unwrap();

    assert_eq!(summary.state, InstallDraftState::Canceled);
